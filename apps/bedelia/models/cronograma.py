...
class CronogramaModel:
    """
    Modelo de Cronograma con validaciones para Rules Engine
    ✅ USA IDs (ObjectId) en vez de nombres de carreras
    """
    ...
    def validar_datos(data: Dict[str, Any], es_actualizacion: bool = False) -> Dict[str, Any]:
        """
        Valida los datos de entrada para crear/actualizar un cronograma
        ✅ CORRECCIÓN: id_carrera ahora es ObjectId, no String
        ...
        # Validaciones obligatorias para creación
        if not es_actualizacion:
            # ✅ CORRECCIÓN: id_aula es ObjectId
            if "id_aula" not in data:
                errores.append("Campo 'id_aula' es obligatorio")
            elif not isinstance(data["id_aula"], ObjectId):
                try:
            ...
            # ✅ CORRECCIÓN: id_materia es ObjectId
            if "id_materia" not in data:
                errores.append("Campo 'id_materia' es obligatorio")
            elif not isinstance(data["id_materia"], ObjectId):
                try:
            ...
            # ✅ CORRECCIÓN: id_profesor es ObjectId
            if "id_profesor" not in data:
                errores.append("Campo 'id_profesor' es obligatorio")
            elif not isinstance(data["id_profesor"], ObjectId):
                try:
            ...
            # ✅ CORRECCIÓN: id_carrera es ObjectId (antes era String)
            if "id_carrera" not in data:
                errores.append("Campo 'id_carrera' es obligatorio")
            elif not isinstance(data["id_carrera"], ObjectId):
                try:
                    data["id_carrera"] = ObjectId(data["id_carrera"])
                except:
                    errores.append("'id_carrera' debe ser un ObjectId válido")
        ...
        if not es_actualizacion:
            ...
            documento = {
                ...
                "id_materia": data["id_materia"],
                "id_profesor": data["id_profesor"],
                "id_carrera": data["id_carrera"],  # ✅ CORRECCIÓN: ObjectId, no String
                "fecha": fecha_valor,
                "hora_inicio": data["hora_inicio"],
                "hora_fin": data["hora_fin"],
                "duracion_minutos": duracion,
            ...
            for campo in campos_permitidos:
                if campo in data:
                    ...
                    if campo in ["id_aula", "id_materia", "id_profesor", "id_carrera"]:
                        if not isinstance(data[campo], ObjectId):
                            try:
                                documento[campo] = ObjectId(data[campo])
                            except:
                                raise ValueError(f"'{campo}' debe ser un ObjectId válido")
                        else:
                            documento[campo] = data[campo]
                    else:
                        documento[campo] = data[campo]
    ...
    def crear_indices(coleccion):
        ...
        # ✅ CORRECCIÓN: índice por id_carrera (ObjectId)
        coleccion.create_index(
            [("id_carrera", 1)],
            name="idx_carrera"
        )
    ...
    @staticmethod
    def listar_por_carrera_materia(coleccion, id_carrera: ObjectId, id_materia: ObjectId) -> List[Dict[str, Any]]:
        """
        Lista cronogramas por carrera y materia (para alumnos)
        ✅ CORRECCIÓN: id_carrera ahora es ObjectId
        
        Args:
            coleccion: Colección MongoDB
            id_carrera: ObjectId de la carrera
            id_materia: ObjectId de la materia
        ...
        """
        return list(coleccion.find({
            "id_carrera": id_carrera,  # ✅ CORRECCIÓN: ObjectId
            "id_materia": id_materia,
            "estado": {"$in": ["programada", "activa"]}
        }).sort("fecha", 1))
